...
class CuroboMotionPlanner:
    def __init__(self, cfg):        
        ...
        self.world_cfg = self.load_stage_simple()
    ...
    def setup_ik_solver(self):
        ...
        return IKSolver(ik_solver_config)
    def setup_ik(self):
        ik_solver_config = IKSolverConfig.load_from_robot_config(
            self.robot_cfg,
            self.world_cfg,
        ...
        return IKSolver(ik_solver_config)
    @staticmethod
    def convert_wxyz_poses_to_curobo(poses: List[List[float]]) -> Pose:
        """Converts xyzw poses to CuRobo's Pose format."""
        poses = np.array(poses)
        pos = torch.from_numpy(poses[:,:3].copy())
        quat = torch.from_numpy(poses[:,[3,4,5,6]].copy())
    ...
    def compute_kinematics_batch(self, 
        ...
        batch = n_seeds
        joint_state_config = joint_state.clone().repeat(n_seeds, 1)
        initial_seed = joint_state.clone().unsqueeze(0).repeat(n_seeds, batch, 1)


        goal_poses = self.convert_wxyz_poses_to_curobo(goal_poses)
        batch_solver = self.setup_ik_solver()
        ik_results = batch_solver.solve_any(
            solve_type=ReacherSolveType.BATCH,
            retract_config=joint_state_config,
    ...
    def convert_xyzw_poses_to_curobo(poses: List[List[float]]) -> Pose:
        ...
        return pose
    def generate_yaw_poses(self, goal_ee_pose: List[float]) -> List[List[float]]:
        """
        ...
        """
        position = goal_ee_pose[:3]
        quaternion = goal_ee_pose[3:]
        print(position)
        print(quaternion)
    ...
    def compute_intermediary_pose(self, goal_ee_pose: List[float], offset_z: float):
        ...
        # )
        intermediary_pose = intermediary_position.squeeze(0).cpu().numpy().tolist() + quaternion.squeeze(0).cpu().numpy().tolist()
    ...
    def ik_goal_generator(self, initial_js, waypoint_pose, target_pose):
        initial_joint_state = self.tensor_args.to_device([initial_js])
        initial_seed = initial_joint_state.detach().clone().unsqueeze(0)
        goal_pose = Pose(
            position=self.tensor_args.to_device(target_pose[0:3]),
            quaternion=self.tensor_args.to_device(target_pose[3:]),
        )
        goal_ik_solver = self.setup_ik()
        goal_ik_solver.update_pose_cost_metric(self.freespace_cost)

        ik_goal = goal_ik_solver.solve_single(
                    goal_pose=goal_pose,
                    retract_config=initial_joint_state,
                    seed_config=initial_seed,
                    )
        success_goal = np.sum(ik_goal.success.cpu().numpy()) > 0
        if not success_goal:
            print("Target Pose is NOT reachable")
            return None, None

        # NOW FIND IK FROM GOAL TO INTERMEDIARY
        # goal is the initial seed now

        goal_joint_state = ik_goal.js_solution.position.squeeze() # CHECK SIZE

        goal_seed = goal_joint_state.detach().clone().unsqueeze(0)

        intermediary_pose = Pose(
            position=self.tensor_args.to_device(waypoint_pose[0:3]),
            quaternion=self.tensor_args.to_device(waypoint_pose[3:])
        )
        waypoint_ik_solver = self.setup_ik()
        waypoint_ik_solver.update_pose_cost_metric(self.linear_cost)
        ik_intermediary = waypoint_ik_solver.solve_single(
                    goal_pose=intermediary_pose,
                    retract_config=goal_joint_state,
                    seed_config=goal_seed,
        )

        success_intermediary = np.sum(ik_intermediary.success.cpu().numpy()) > 0
        if not success_intermediary:
            print("Intermediary Pose solution not found")
            return None, None
        ik_solution_intermediary = ik_intermediary.js_solution.position.cpu().squeeze().tolist()
        ik_solution_goal = ik_goal.js_solution.position.cpu().squeeze().tolist()
        print("intermediary ik solution: ")
        print(ik_solution_intermediary)
        
        return ik_solution_intermediary, ik_solution_goal #python list of js 
    ...
    #     # goal_js_pose = [1.3056849, 1.4040100, -0.34258141, 1.743283, 0.017052, 1.627947, -0.129718]
    #     # goal_js_pose = self.q_start
    #     inter_js_pose = JointState.from_position(
    #         position=self.tensor_args.to_device([intermediary_js]),
    #         joint_names=self.j_names[0 : len(intermediary_js)],
    #     )
        
    #     try:
    #         self.release_constraint()
    #         motion_gen_result_1 = self.motion_gen.plan_single_js(
    #             initial_js, inter_js_pose, self.plan_config
    #         )
    #         reach_succ_1 = motion_gen_result_1.success.item()
    #         print("Success (should only have one): ", reach_succ_1)
    #         interpolated_solution_1 = motion_gen_result_1.get_interpolated_plan() 
    #     except Exception as e:
            
    #         print("Error in planning trajectory: ", e)
    #         return None

    #     goal_js_pose = JointState.from_position(
    #         position=self.tensor_args.to_device([goal_js]),
    #         joint_names=self.j_names[0 : len(goal_js)],
    #     )
        
    #     try: 
    #         self.world_model.enable_obstacle("world", False)
    #         self.set_constraint()
    #         motion_gen_result_2 = self.motion_gen.plan_single_js(
    #             inter_js_pose, goal_js_pose, self.plan_config
    #         )
    #         reach_succ_2 = motion_gen_result_2.success.item()
    #         print("Success (should only have one): ", reach_succ_2)
    #         interpolated_solution_2 = motion_gen_result_2.get_interpolated_plan()
    #     except Exception as e:
    #         print("Error in planning trajectory: ", e)
    #         return None
        
        
    #     if reach_succ_1 and reach_succ_2:      
    #         # Concatenate the two trajectories
    #         combined_positions = interpolated_solution_1.position.cpu().numpy().tolist() + interpolated_solution_2.position.cpu().numpy().tolist()
    #         combined_velocities = interpolated_solution_1.velocity.cpu().numpy().tolist() + interpolated_solution_2.velocity.cpu().numpy().tolist()
    #         combined_accelerations = interpolated_solution_1.acceleration.cpu().numpy().tolist() + interpolated_solution_2.acceleration.cpu().numpy().tolist()
    #         combined_jerks = interpolated_solution_1.jerk.cpu().numpy().tolist() + interpolated_solution_2.jerk.cpu().numpy().tolist()
    #         combined_interpolation_dt = motion_gen_result_1.interpolation_dt + motion_gen_result_2.interpolation_dt

    #         solution_dict = {
    #             "success": True,
    #             "joint_names": interpolated_solution_1.joint_names,
    #             "positions": combined_positions,
    #             "velocities": combined_velocities,
    #             "accelerations": combined_accelerations,
    #             "jerks": combined_jerks,
    #             "interpolation_dt": combined_interpolation_dt,
    #             "raw_data": (interpolated_solution_1, interpolated_solution_2),
    #         }
            
    #         return solution_dict
    #     else:
    #         print("Failed to reach goal or intermediary")
    #         return None
